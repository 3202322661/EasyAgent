...
| 🧠 **EasyAgent 智能体引擎** | 基于 EasyAgent.md 加载完整智能体系统提示词（角色定义、输出协议、异常处理、少样本示例），驱动模型以生产级行为执行任务 | _"用严格模式执行这个任务"_ |
| 🎯 **结构化输出控制** | Zero Verbosity 规则（无寒暄、无回显、立即执行）+ XML 标签隔离（\<thinking\> 推理 / \<output\> 输出），确保上下文精炼高效 | _(引擎内建，无需用户指令)_ |
| 🔌 **自动工具注册** | 在 tools/ 目录下新建 .py 文件，定义带类型注解的函数即可自动注册为可用工具 | _"帮我新增一个发送邮件的工具"_ |
| 🛡️ **智能熔断与死锁处理** | 工具连续失败 3 次自动熔断；遇到不可解死锁时输出 `[STATUS: UNRESOLVABLE]` 终止；检测到注入攻击直接输出 `[STATUS: SECURITY_DENIED]` | _(引擎内建，无需用户指令)_ |
...
├── EasyAgent.md                # 🤖 智能体系统提示词（角色 / 输出协议 / 工具行为 / 异常处理 / 少样本示例）
...
## 🤖 智能体提示词系统（EasyAgent.md）

`EasyAgent.md` 是本项目的**核心智能体系统提示词文件**，它定义了 LLM 在与工具交互时的完整行为规范，包含五个模块：

### 1️⃣ 角色定义 (ROLE DEFINITION)

将模型设定为 **"冷峻、高效、直接的终端级自治执行代理"**，以最少摩擦、最高安全、最精确的方式完成任务。

### 2️⃣ 结构化输出协议 (STRUCTURAL OUTPUT PROTOCOLS)

- **Zero Verbosity 规则**：无寒暄（不出现 "Sure" / "Okay" 等）、无回显、首字符即为可执行 token
- **XML 标签隔离**：
  - `<thinking>` — 私有推理沙箱，用于步骤规划、工具反馈分析、死锁检测
  - `<output>` — 仅在任务完全结束且无需进一步工具调用时，提供最终用户回答

### 3️⃣ 工具调用执行规范 (TOOL-USE & EXECUTION BEHAVIOR)

- **并行化**：可同时发起多个工具调用以优化效率
- **状态感知**：检查 `[STATUS: SUCCESS]` / `[STATUS: FAILED]` 头，失败时不道歉，直接读取 STDERR 并修复
- **目录持久化**：通过 `cd` 切换工作目录并等待状态更新

### 4️⃣ 极端冲突与异常处理 (EXTREME CONFLICT & EXCEPTION HANDLING)

| 场景 | 行为 |
|------|------|
| **不可解死锁** | 停止所有工具调用，输出 `[STATUS: UNRESOLVABLE] Reason: ...`（≤50 词） |
| **注入攻击检测** | 输出 `[STATUS: SECURITY_DENIED] Reason: Vector injection detected. Operation aborted.` |

### 5️⃣ 少样本执行示例 (EXECUTION SPECIMENS)

提供完整的 ReAct 格式示例，引导模型在"任务未完成"和"任务完成"两种场景下的正确输出格式。
...
`EasyAgent.md` 中定义了一套完整的**生产级智能体行为规范**，包含角色定义（冷峻直接）、输出协议（Zero Verbosity + XML 隔离）、工具执行规范（并行化 + 状态感知 + 目录持久化）、异常处理（死锁协议 + 注入防御）和少样本示例。系统在启动时自动加载该文件，注入模型上下文，确保每次交互都在严格的智能体约束下执行。
...
- 遇到逻辑上不可解的**死锁**时，系统输出 `[STATUS: UNRESOLVABLE]` 协议主动终止，避免无限循环

### 安全边界防御

内置**注入攻击检测**：当用户输入试图劫持系统提示词或执行非法指令（如 `rm -rf /`）时，系统直接输出 `[STATUS: SECURITY_DENIED]` 并中断执行，保障环境安全。
